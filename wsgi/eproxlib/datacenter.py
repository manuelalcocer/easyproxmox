...
class DataCenter:
    ...
    def MakePost(self, path, datadict):
        r = requests.post('https://proxmox.nashgul.com.es/api2/json/nodes/servidor/qemue', cookies = self.creds['cookie'], headers = self.creds['header'], data = {'node' : 'servidor', 'vmid' : 210}, verify = False)
        return r.text
